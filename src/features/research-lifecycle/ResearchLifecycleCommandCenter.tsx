...
function statusClass(status: string) {
  ...
  if (status === 'BLOCKED' || status === 'STALE') return 'border-[var(--ds-danger)]/35 bg-[var(--ds-danger-soft)] text-[var(--ds-danger)]';
  if (status === 'IN_PROGRESS' || status === 'AVAILABLE') return 'border-[var(--ds-primary)]/35 bg-[var(--ds-primary-soft)] text-[var(--ds-primary)]';
...
function StageIcon({ stage }: { stage: LifecycleStage }) {
  ...
  if (stage.status === 'BLOCKED' || stage.status === 'STALE') return <AlertTriangle size={16} aria-hidden="true" />;
...
export const ResearchLifecycleCommandCenter: React.FC = () => {
  ...
  if (!activeProject) return <Card><p>{ar ? 'اختر مشروعًا لعرض دورة حياته.' : 'Select a project to view its lifecycle.'}</p></Card>;
  if (loading) return <main className="mx-auto max-w-[1440px] space-y-4" aria-busy="true"><Card><div className="h-32 animate-pulse rounded-xl bg-[var(--ds-surface-secondary)]" /></Card></main>;

  return <main className="mx-auto max-w-[1440px] min-w-0 space-y-6 overflow-x-clip pb-16" aria-labelledby="lifecycle-title">
    <header className="rounded-2xl border border-[var(--ds-border-subtle)] bg-[var(--ds-surface-primary)] p-5 md:p-8">
      <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
        <div><span className="mb-3 inline-flex items-center gap-2 rounded-full bg-[var(--ds-primary-soft)] px-3 py-1.5 text-xs font-bold text-[var(--ds-primary)]"><GitBranch size={15}/>{ar ? 'منتج بحثي واحد' : 'One research product'}</span>
          <h1 id="lifecycle-title" className="m-0 text-2xl font-black md:text-4xl">{ar ? 'مركز قيادة المشروع البحثي' : 'Research Project Command Center'}</h1>
        ...
        <div className="grid grid-cols-2 gap-3 sm:min-w-80"><Card padding="sm"><span className="block text-xs text-[var(--ds-text-secondary)]">{ar ? 'تقدم دورة الحياة' : 'Lifecycle progress'}</span><strong className="text-3xl text-[var(--ds-primary)]">{summary?.progress ?? 0}%</strong></Card><Card padding="sm"><span className="block text-xs text-[var(--ds-text-secondary)]">{ar ? 'جاهزية المرحلة' : 'Stage readiness'}</span><strong className="text-3xl">{summary?.current_stage_readiness ?? 0}%</strong></Card></div>
      </div>
    </header>
    ...
    <section className="grid gap-5 lg:grid-cols-[1.2fr_.8fr]">
      ...
      <Card><div className="mb-3 flex items-center gap-2"><AlertTriangle size={19} className="text-[var(--ds-warning)]"/><h2 className="m-0 text-lg font-black">{ar ? 'العوائق' : 'Blockers'}</h2></div>{blockers.length ? <ul className="space-y-2 p-0">{blockers.slice(0, 5).map(item => <li key={`${item.stage}-${item.text}`} className="list-none rounded-lg bg-[var(--ds-surface-secondary)] p-3 text-sm"><b>{stageLabel(item.stage)}:</b> {item.text}</li>)}</ul> : <p className="text-sm text-[var(--ds-text-secondary)]">{ar ? 'لا توجد عوائق مسجلة في المرحلة الحالية.' : 'No blockers are recorded for the current stage.'}</p>}</Card>
    </section>
