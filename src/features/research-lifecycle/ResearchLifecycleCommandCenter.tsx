...
export const ResearchLifecycleCommandCenter: React.FC = () => {
  ...
  return <section className="mx-auto max-w-[1440px] min-w-0 space-y-6 overflow-x-clip pb-16" aria-labelledby="lifecycle-title">
    ...
    <header>
      <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
        <div><span className="mb-3 inline-flex items-center gap-2 rounded-full bg-[var(--ds-primary-soft)] px-3 py-1.5 text-xs font-bold text-[var(--ds-primary)]"><GitBranch size={15}/>{ar ? 'منتج بحثي واحد' : 'One research product'}</span>
          <h2 id="lifecycle-title" className="text-h2 m-0">{ar ? 'مركز قيادة المشروع البحثي' : 'Research Project Command Center'}</h2>
          <p className="text-body-sm mt-2 font-bold text-[var(--ds-text-primary)]">{ar ? activeProject.titleAr : activeProject.titleEn}</p>
          <p className="text-body-sm mt-1 text-[var(--ds-text-secondary)]">{summary ? `${stageLabel(summary.current_stage)} · ${summary.template.replaceAll('_', ' ')}` : ''}</p>
        </div>
    ...
    <section aria-labelledby="rail-title"><div className="mb-3 flex items-center justify-between"><h2 id="rail-title" className="text-h2 m-0">{ar ? 'مسار دورة الحياة' : 'Lifecycle rail'}</h2><Button variant="ghost" size="sm" iconBefore={<RefreshCw size={15}/>} onClick={load}>{ar ? 'تحديث' : 'Refresh'}</Button></div>
      <div className="overflow-x-auto rounded-2xl border border-[var(--ds-border-subtle)] bg-[var(--ds-surface-primary)] p-4" role="region" tabIndex={0} aria-label={ar ? 'مراحل دورة حياة المشروع' : 'Project lifecycle stages'}>
        <ol className="flex min-w-max items-start gap-2 p-0" aria-label={ar ? 'تسلسل المراحل' : 'Stage sequence'}>{summary?.stages.map((stage, index) => <li key={stage.key} className="flex items-center"><button onClick={() => document.getElementById(`stage-${stage.key}`)?.scrollIntoView({ behavior: 'smooth', block: 'center' })} className={`flex w-36 flex-col items-center gap-2 rounded-xl border p-3 text-center ${statusClass(stage.status)}`} aria-current={stage.key === summary.current_stage ? 'step' : undefined}><span className="grid h-8 w-8 place-items-center rounded-full border border-current"><StageIcon stage={stage}/></span><span className="text-xs font-black">{stageLabel(stage.key)}</span><span className="text-caption">{statusLabels[stage.status]?.[ar ? 0 : 1] ?? stage.status}</span></button>{index < summary.stages.length - 1 && <span className="mx-1 h-px w-6 bg-[var(--ds-border-strong)]" aria-hidden="true"/>}</li>)}</ol>
      </div>
    </section>

    <section className="grid gap-5 lg:grid-cols-[1.2fr_.8fr]">
      <Card variant={summary?.next_action.priority === 'CRITICAL' ? 'warning' : 'ai-accent'}><div className="mb-3 flex items-center gap-2 text-[var(--ds-primary)]"><Sparkles size={20}/><h2 className="text-h2 m-0">{ar ? 'الإجراء الأكاديمي التالي' : 'Next academic action'}</h2></div><span className="rounded-full bg-[var(--ds-surface-primary)] px-3 py-1 text-xs font-black">{summary?.next_action.priority}</span><h3 className="text-h3 mt-5">{summary?.next_action.title}</h3><p className="text-body-sm text-[var(--ds-text-secondary)]">{summary?.next_action.rationale}</p><div className="mt-4 flex items-center gap-2 text-xs text-[var(--ds-text-secondary)]"><ShieldCheck size={16}/>{ar ? 'أولوية حتمية؛ الذكاء الاصطناعي لا يغيّر الحالة' : 'Deterministic priority; AI cannot change state'}</div></Card>
      <Card><div className="mb-3 flex items-center gap-2"><AlertTriangle size={19} className="text-[var(--ds-warning)]"/><h2 className="text-h2 m-0">{ar ? 'العوائق' : 'Blockers'}</h2></div>{blockers.length ? <ul className="space-y-2 p-0">{blockers.slice(0, 5).map(item => <li key={`${item.stage}-${item.text}`} className="list-none rounded-lg bg-[var(--ds-surface-secondary)] p-3 text-sm"><b>{stageLabel(item.stage)}:</b> {item.text}</li>)}</ul> : <EmptyState bare className="py-4" title={ar ? 'لا توجد عوائق مسجلة' : 'No blockers recorded'} description={ar ? 'المرحلة الحالية بلا عوائق موثّقة.' : 'The current stage has no documented blockers.'}/>}</Card>
    </section>
