...
export const ResearchDataStudio: React.FC = () => {
  ...
  return <section className="mx-auto max-w-[1440px] min-w-0 space-y-6 overflow-x-clip pb-16" aria-labelledby="studio-title">
    ...
    <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3" aria-label={ar?'مؤشرات مستقلة':'Independent indicators'} data-testid="rdcc-indicators">
      ...
      <Card padding="sm" variant={next.priority==='CRITICAL'||next.priority==='HIGH'?'warning':'ai-accent'} data-testid="ind-next-action"><div className="flex items-center gap-2"><Play size={16} className="text-[var(--ds-primary)]"/><span className="text-xs font-bold">{ar?'الإجراء التالي':'Next action'}</span></div><p className="text-body-sm font-bold mt-1">{nextTitle}</p><span className="inline-block mt-1 rounded-full bg-[var(--ds-surface-primary)] px-2 py-0.5 text-[10px] font-black">{next.priority}</span></Card>
    </section>
