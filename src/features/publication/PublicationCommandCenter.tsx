  ...
  return <section dir={ar ? 'rtl' : 'ltr'} className="mx-auto max-w-7xl min-w-0 space-y-5" aria-labelledby="publication-title">
    <PathPanel accent="var(--ds-path-publication)">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <p className="mb-2 text-xs font-black text-path-publication">BASEERAH · PUBLICATION INTELLIGENCE</p>
          <h2 id="publication-title" className="m-0 text-2xl font-black text-ink sm:text-3xl">{ar ? 'مركز ذكاء النشر العلمي' : 'Publication Intelligence Command Center'}</h2>
          <p className="mt-2 max-w-3xl text-sm text-secondary">{ar ? 'جاهزية المخطوطة وامتثال الإبلاغ وملاءمة المجلات ومسار التقديم—كل مقياس مستقل وقابل للتفسير.' : 'Manuscript readiness, reporting compliance, journal suitability, and submission workflow—kept separate and explainable.'}</p>
        </div>
    ...
    {center && <>
      ...
      <section className="grid gap-4 lg:grid-cols-[1.35fr_.65fr]">
        <article className="rounded-2xl border border-[var(--ds-border-default)] bg-[var(--ds-surface-primary)] p-5">
          <div className="flex items-center justify-between">
            <h2 className="font-black">{ar ? 'بنية المخطوطة' : 'Manuscript outline'}</h2>
          ...
          {center.version && <p className="mt-1 text-xs text-[var(--ds-text-muted)]">v{center.version.number} · {center.version.article_type}</p>}
        ...
        <aside className="rounded-2xl border border-[var(--ds-border-default)] bg-[var(--ds-surface-primary)] p-5">
          <h2 className="font-black">{ar ? 'الإجراء التالي' : 'Next best action'}</h2>
          <p className="mt-4 rounded-xl bg-warning/10 p-4 text-sm font-bold"><span className="mb-1 block text-xs text-warning">{center.next_best_action.priority}</span>{String(center.next_best_action.code || '')}</p>
          <p className="mt-4 text-xs leading-6 text-[var(--ds-text-muted)]">{ar ? 'اختيار المجلة والتقديم والنشر قرارات بشرية. درجة الملاءمة لا تمثل احتمال القبول.' : 'Journal selection, submission, and publication remain human decisions. Match never represents acceptance probability.'}</p>
      ...
      {tab === 'journals' && (
        <article className="rounded-2xl border border-[var(--ds-border-default)] bg-[var(--ds-surface-primary)] p-5 space-y-4">
          <h2 className="m-0 font-black">{ar ? 'القائمة المختصرة للمجلة' : 'Journal shortlist'}</h2>
          <p className="text-xs text-[var(--ds-text-muted)]">{ar ? 'الملاءمة دعم للقرار وليست احتمال قبول.' : 'Suitability is decision support, not acceptance probability.'}</p>
          ...
          {journals.length === 0 ? <p className="text-xs text-[var(--ds-text-muted)]">{ar ? 'كتالوج المجلات فارغ حتى يضيفه مدير المؤسسة.' : 'The journal catalog is empty until an organization admin adds titles.'}</p> : (
      ...
      {tab === 'authorship' && (
        <article className="rounded-2xl border border-[var(--ds-border-default)] bg-[var(--ds-surface-primary)] p-5 space-y-4">
          <div className="flex items-center gap-2"><Users size={18} /><h2 className="m-0 font-black">{ar ? 'التأليف وCRediT' : 'Authorship and CRediT'}</h2></div>
            ...
            <>
              {(authorship?.authors || []).map((author: any) => (
                <div key={author.id || author.user_id} className="rounded-xl bg-[var(--ds-surface-secondary)] p-3 text-sm">
                  ...
                  <p className="mt-1 text-xs">{ar ? `الترتيب ${author.author_order}` : `Order ${author.author_order}`} · {(author.credit_roles || []).join(', ') || '—'}</p>
                </div>
              ))}
      ...
      {tab === 'submissions' && (
        <article className="rounded-2xl border border-[var(--ds-border-default)] bg-[var(--ds-surface-primary)] p-5 space-y-4">
          <h2 className="m-0 font-black">{ar ? 'مسار التقديم' : 'Submission track'}</h2>
