    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <Card className="p-4 flex items-center gap-3.5 border-[var(--ds-border-subtle)] bg-[var(--ds-surface-primary)]">
          <div className={`h-9 w-9 rounded-xl flex items-center justify-center shrink-0 ${status?.database === 'ready' ? 'bg-action/10 text-success' : 'bg-danger/10 text-danger'}`}>
            <Database size={18} />
          </div>
          <div>
            ...
            <div className="flex items-center gap-1.5 mt-0.5">
              ...
              <span className={`px-1.5 py-0.5 rounded-md text-[8px] font-black uppercase ${status?.database === 'ready' ? 'bg-action/10 text-success' : 'bg-danger/10 text-danger'}`}>
        ...
        <Card className="p-4 flex items-center gap-3.5 border-[var(--ds-border-subtle)] bg-[var(--ds-surface-primary)]">
          <div className="h-9 w-9 bg-info/10 text-path-publication rounded-xl flex items-center justify-center shrink-0">
            <Activity size={18} />
          </div>
          <div>
            <div className="text-[10px] text-[var(--ds-text-muted)] font-black uppercase tracking-wider">
            ...
            <div className="flex items-center gap-1.5 mt-0.5">
              ...
              <span className="px-1.5 py-0.5 rounded-md bg-action/10 text-success text-[8px] font-black uppercase">{status?.database === 'ready' ? (language === 'ar' ? 'متصل' : 'ONLINE') : (language === 'ar' ? 'غير متصل' : 'OFFLINE')}</span>
            </div>
          </div>
        </Card>
        ...
        <Card className="p-4 flex items-center gap-3.5 border-[var(--ds-border-subtle)] bg-[var(--ds-surface-primary)]">
          ...
          <div>
            ...
            <div className="flex items-center gap-1.5 mt-0.5">
              ...
              <span className={`px-1.5 py-0.5 rounded-md text-[8px] font-black uppercase ${status?.ai_provider?.includes('LIVE') ? 'bg-action/10 text-success' : 'bg-warning/10 text-warning'}`}>
        ...
        <Card className="p-4 flex items-center gap-3.5 border-[var(--ds-border-subtle)] bg-[var(--ds-surface-primary)]">
          ...
          <div>
            ...
            <div className="flex items-center gap-1.5 mt-0.5">
              ...
              <span className="px-1.5 py-0.5 rounded-md bg-action/10 text-success text-[8px] font-black uppercase">{status?.counts?.ai_runs ?? 0} {language === 'ar' ? 'تشغيل' : 'runs'}</span>
            </div>
          </div>
        </Card>
      </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 space-y-6">
              <Card className="p-6 border-[var(--ds-border-subtle)] bg-[var(--ds-surface-primary)] rounded-3xl space-y-4">
                ...
                {saveMessage && (
                  <div className={`px-3 py-2 rounded-xl text-xs font-bold ${saveMessage.includes('نجاح') || saveMessage.includes('success') ? 'bg-action/10 text-success' : 'bg-danger/10 text-danger'}`}>
                    {saveMessage}
                  </div>
                )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs font-bold text-secondary">
                  <div className="space-y-1">
                    ...
                    <input type="text" value={platformTitleAr} onChange={e => setPlatformTitleAr(e.target.value)}
                      className="w-full bg-[var(--ds-surface-secondary)] border border-[var(--ds-border-subtle)] rounded-xl px-3.5 py-2.5 text-ink" />
                  </div>
                  <div className="space-y-1">
                ...
                <div className="space-y-1">
                  ...
                  <textarea value={announcementAr} onChange={e => setAnnouncementAr(e.target.value)} rows={2}
                    className="w-full bg-[var(--ds-surface-secondary)] border border-[var(--ds-border-subtle)] rounded-xl px-3.5 py-2.5 text-ink text-xs" />
                </div>
                <div className="space-y-1">
              ...
              <Card className="p-6 border-[var(--ds-border-subtle)] bg-[var(--ds-surface-primary)] rounded-3xl space-y-4">
                <div>
                  <h3 className="text-sm font-black text-ink m-0 flex items-center gap-2">
                    <Layers size={16} className="text-ai" />
                    ...
                    return (
                      <div key={key} className="p-3 rounded-xl border border-[var(--ds-border-subtle)] bg-[var(--ds-surface-secondary)]/50 flex justify-between items-center gap-4 hover:border-subtle transition-all">
                        ...
                        <button type="button" onClick={() => toggleFlag(key)}
                          className="text-[var(--ds-text-secondary)] hover:text-ai transition-colors bg-transparent border-none cursor-pointer">
                          {currentValue ? <ToggleRight size={28} className="text-ai" /> : <ToggleLeft size={28} className="text-secondary" />}
                        </button>
                      </div>
                    );
                  })}
            ...
            <div className="lg:col-span-1 space-y-6">
              <Card className="p-6 border-[var(--ds-border-subtle)] bg-canvas rounded-3xl space-y-4 text-xs font-semibold text-muted leading-relaxed">
                <h4 className="text-xs font-black text-ink m-0 uppercase tracking-wider flex items-center gap-1.5">
                  <Server size={14} className="text-ai" />
                  <span>{language === 'ar' ? 'حالة النظام' : 'System Status'}</span>
                </h4>
                <div className="space-y-2.5">
                  {[
                    { labelAr: 'إصدار المنصة', labelEn: 'App Version', value: `v${status?.version ?? '—'}`, color: 'text-ai' },
                    ...
                    { labelAr: 'الجهات (المستأجرين)', labelEn: 'Organizations', value: String(status?.counts?.organizations ?? 0), color: 'text-ai' },
                    { labelAr: 'المستخدمين', labelEn: 'Users', value: String(status?.counts?.users ?? 0), color: 'text-ai' },
                    { labelAr: 'المشاريع البحثية', labelEn: 'Projects', value: String(status?.counts?.projects ?? 0), color: 'text-ai' },
                    { labelAr: 'تشغيلات الذكاء الاصطناعي', labelEn: 'AI Runs', value: String(status?.counts?.ai_runs ?? 0), color: 'text-ai' },
                    { labelAr: 'المراجعات الأخيرة', labelEn: 'Recent Audit Events', value: String(status?.recent_audit_count ?? 0), color: 'text-ai' },
                  ].map((item, i) => (
              ...
              <Card className="p-6 border-[var(--ds-border-subtle)] bg-[var(--ds-surface-primary)] rounded-3xl text-xs text-muted space-y-2">
                <div className="text-ai font-black flex items-center gap-1">
                  <Sparkles size={14} />
                ...
                <button onClick={loadAll} className="flex items-center gap-1.5 text-[10px] font-black text-ai hover:text-ai transition-colors cursor-pointer">
                  <RefreshCw size={12} />
