                  ...
                  <div className="flex gap-1">
                    {[0.80, 0.90, 0.95].map((pVal) => (
                      <button
                        ...
                        className={`px-1.5 py-0.5 rounded text-[8px] font-extrabold cursor-pointer border ${power === pVal ? 'bg-[var(--ds-primary-soft)] text-ink border-[var(--ds-primary)]' : 'bg-[var(--ds-surface-secondary)] text-[var(--ds-text-muted)] border-[var(--ds-border-subtle)] hover:bg-[var(--ds-surface-tertiary)]'}`}
                      >
                        {pVal}
                      </button>
                    ))}
              ...
              <div className="flex gap-1.5">
                {effectSizePresets.map((preset) => (
                  <button
                    ...
                    className={`px-2 py-0.5 rounded text-[8px] font-extrabold cursor-pointer border ${effectSize === preset.val ? 'bg-[var(--ds-primary-soft)] text-ink border-[var(--ds-primary)]' : 'bg-[var(--ds-surface-secondary)] text-[var(--ds-text-muted)] border-[var(--ds-border-subtle)] hover:bg-[var(--ds-surface-tertiary)]'}`}
                  >
            ...
            <div className="p-4 bg-[var(--ds-surface-secondary)] border border-[var(--ds-border-subtle)] rounded-xl text-xs space-y-3">
              ...
              <div className="space-y-2 pt-1.5 border-t border-[var(--ds-border-subtle)] font-medium text-[11px] text-[var(--ds-text-secondary)]">
                <div className="flex justify-between">
                  <span className="text-muted">{language === 'ar' ? 'التصميم المستهدف' : 'Target Design'}</span>
                ...
                {testType === 'anova_one_way' ? (
                  <>
                    <div className="flex justify-between">
                      <span className="text-muted">{language === 'ar' ? 'عدد المجموعات' : 'Groups'}</span>
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={chartData} margin={{ top: 15, right: 15, left: -20, bottom: 0 }}>
                    ...
                    <Line 
                      ...
                      strokeWidth={3} 
                      dot={false}
                      activeDot={{ r: 6, fill: 'var(--ds-chart-2)' }}
                    />
                    <ReferenceLine 
