...
def upgrade():
    ...
    if "uq_reviewer_assignment_internal" not in existing_constraints:
        op.create_unique_constraint("uq_reviewer_assignment_internal", "reviewer_assignments", ["round_id", "reviewer_user_id"])
    if "uq_reviewer_assignment_external" not in existing_constraints:
        op.create_unique_constraint("uq_reviewer_assignment_external", "reviewer_assignments", ["round_id", "external_email"])
    ...
    if "uq_manuscript_revision_version" not in revision_constraints:
        op.create_unique_constraint("uq_manuscript_revision_version", "manuscript_revisions", ["case_id", "version_number"])


def downgrade():
    op.drop_constraint("uq_manuscript_revision_version", "manuscript_revisions", type_="unique")
    op.drop_constraint("uq_reviewer_assignment_external", "reviewer_assignments", type_="unique")
    op.drop_constraint("uq_reviewer_assignment_internal", "reviewer_assignments", type_="unique")
    with op.batch_alter_table("peer_review_cases") as batch_op:
